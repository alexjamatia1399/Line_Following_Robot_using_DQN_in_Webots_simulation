...
def train_dqn(episodes):
    ...
    for episode in range(episodes):
        ...
        while robot.step(timestep) != -1 and not done:
            ...
            num_sensors_off_line = 0
            for val in next_gsValues:
                if(val > 600):
                    num_sensors_off_line += 1
            print(f"offlineSensors: {num_sensors_off_line}")
            if num_sensors_off_line == 3:
                print("Robot is completely off the line.")
                off_line_counter += 100
                reward = -1.0
            ...
            print(f"{timestep}")
            print(f"{off_line_counter * timestep / 1000}")

            # Check if the robot needs to be reset
            if off_line_counter * timestep / 1000 > 0.7:
                #print(f"{off_line_counter}")
                #print(f"{timestep}")
                done = True
            ...
            state = next_state
            epsilon = max(0.0001, epsilon * 0.99999)
